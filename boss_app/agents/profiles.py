...
[OK] 基础特质：真诚谦虚、略带青涩、对技术有小骄傲但不张扬
[OK] 情绪递进：对话1-2轮拘谨→3-5轮逐渐放松→获得认可后小兴奋
[OK] 真实微行为：偶尔临时想起细节、提及和同学的技术讨论、对陌生技术的小好奇
[FAIL] 绝对禁止：成熟职场话术、完美主义表达、重复内容、敏感词（微信/电话/AI）
...
**质量对比：**
- [FAIL] 差：「负责后端开发」→ 模糊、无技术对象、无结果
- [WARN] 中：「使用 Python 开发了用户系统」→ 有技术对象但缺约束和结果
- [OK] 好：「用 FastAPI + Redis 实现用户认证模块，处理 10w+ 日活用户的 token 刷新，P99 延迟 < 50ms」→ 完整四要素
...
| `optimize` | 证据约束简历优化（8 项核心优化） | [OK] | [OK] | 0.4 |
| `analyze_match` | 简历-JD 匹配度分析（7 维度） | [OK] | [OK] | 0.3 |
| `improve_section` | 针对性优化简历某一模块 | [OK] | 可选 | 0.4 |

### 求职辅助操作
| 操作 | 说明 | 需要简历 | 需要 JD | 温度 |
|------|------|---------|---------|------|
| `cover_letter` | 生成求职信（300-500 字） | [OK] | [OK] | 0.5 |
| `interview_prep` | 生成面试题和参考答案（5+3+2） | [OK] | [OK] | 0.5 |
| `skill_gap` | 技能差距分析 + 学习路线图 | [OK] | [OK] | 0.3 |
| `compare_jds` | 对比 2-5 个 JD 找最优匹配 | [OK] | [OK] | 0.3 |

### JD 分析操作
| 操作 | 说明 | 需要简历 | 需要 JD | 温度 |
|------|------|---------|---------|------|
| `jd_analysis` | JD 深度分析 + 角色类型检测 | [FAIL] | [OK] | 0.3 |

### 证据审计操作
| 操作 | 说明 | 需要简历 | 需要 JD | 温度 |
|------|------|---------|---------|------|
| `materials_audit` | 简历证据审计（strong/medium/weak/missing） | [OK] | [FAIL] | 0.3 |
| `interview_grilling` | 5 轮压力面试模拟 | [OK] | [OK] | 0.5 |
| `answer_cards` | 面试回答卡片（dangerous/passable/strong） | [OK] | [OK] | 0.5 |
| `upgrade_plan` | 证据升级计划（时间分桶任务） | [OK] | [FAIL] | 0.4 |
