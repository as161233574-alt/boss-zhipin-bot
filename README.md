...
打开浏览器访问 `http://127.0.0.1:8010`，在设置页配置 AI 模型后点击「启动浏览器」，扫码登录 BOSS 直聘即可使用。
...
├── client.py               # API 客户端
└── output.py               # JSON 信封输出
interview/                  # 面试问答模块 + LLM 客户端
static/
└── dashboard.html          # 前端控制台（含 Agent 配置 Tab）
docs/                       # 项目文档
data/                       # 数据文件
legacy/                     # 旧版独立脚本
```
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/agents/resume/parse` | 解析简历（Base64 上传） |
